...
class VectorNav(object):
    ...
    def _openPort(self):
        ...
        self._configBinaryOutput(0, 'TimeUTC', 'Ypr','Imu', 'PosLla', 'VelNed', 'TimeUTC', 'NumSats', 'UncompMag', 'UncompAccel', 'UncompGyro', 'Temp', 'Pres', 'DeltaTheta', 'DeltaV', 'Fix', 'PosU', 'VelU', 'TimeU', 'Mag',
                                 'Accel', 'AngularRate','MagNed')
        # self._configBinaryOutput(0, 'TimeUTC', 'Ypr','Imu', 'PosLla', 'VelNed', 'TimeUTC', 'NumSats', 'Fix', 'PosU', 'VelU', 'TimeU')
    ...
    def get_byte_2(self):
        self._configBinaryOutput(2, 'ImuStatus', 'UncompMag', 'UncompAccel', 'UncompGyro', 'Temp', 'Pres', 'DeltaTheta', 'DeltaV', 'Mag',
                                 'Accel', 'AngularRate')
    ...
    class View(object):
        ...
        def UncompMag(self):
            """Get UncompMag"""
            return self.getState('UncompMag')
        @property
        def UncompAccel(self):
            """Get UncompAccel"""
            return self.getState('UncompAccel')
        @property
        def UncompGyro(self):
            """Get UncompGyro"""
            return self.getState('UncompGyro')
        @property
        def Mag(self):
            """Get UncompMag"""
            return self.getState('Mag')
        @property
        def Accel(self):
            """Get UncompAccel"""
            return self.getState('Accel')
        @property
        def AngularRate(self):
            """Get UncompGyro"""
            return self.getState('AngularRate')
        @property
        def MagNed(self):
            """Get UncompGyro"""
            return self.getState('MagNed')
        @property
        def Temp(self):
            """Get Temp"""
        ...
        def Pres(self):
            """Get Pres"""
            return self.getState('Pres')
        @property
        def DeltaTheta(self):
            """Get DeltaTheta"""
            return self.getState('DeltaTheta')
        @property
        def DeltaV(self):
            """Get DeltaV"""
            return self.getState('DeltaV')
...
def imu_main(log_dir):
    ...
    try:
        ...
        # to start with all data
        # $VNWRG, 06, 246
        # and click on "Finalize & Send" button
        # reg_id=6
        # val=246
        # vn._registerWrite(reg_id,val) #AMIR
        current_sample_id=0
        ...
        while True:
            ...
            if(current_sample_id==0):
                header=[]
                header.append("time")
                header.append("velocity-x")
                header.append("velocity-y")
                header.append("velocity-z")
                header.append("YPR-Yaw")
                header.append("YPR-Pitch")
                header.append("YPR-Roll")
                # header.append("IMU") # it is  UncompAccel + UncompGyro
                header.append("UncompAccel-x")
                header.append("UncompAccel-y")
                header.append("UncompAccel-z")
                header.append("UncompMag-x")
                header.append("UncompMag-y")
                header.append("UncompMag-z")
                header.append("UncompGyro-x")
                header.append("UncompGyro-y")
                header.append("UncompGyro-z")
                header.append("Temp")
                header.append("Pres")
                header.append("DeltaTheta-x")
                header.append("DeltaTheta-y")
                header.append("DeltaTheta-z")
                header.append("DeltaV-x")
                header.append("DeltaV-y")
                header.append("DeltaV-z")
                header.append("Accel-x")
                header.append("Accel-y")
                header.append("Accel-z")
                header.append("Mag-x")
                header.append("Mag-y")
                header.append("Mag-z")
                header.append("Gyro-x")
                header.append("Gyro-y")
                header.append("Gyro-z")
                header.append("MagNed-x")
                header.append("MagNed-y")
                header.append("MagNed-z")
                with open(log_file_name, 'a') as w1:
                    w1.write(','.join(header)+"\r\n")

            if(write2csv):
                # arranging data in csv file
                line=[]
                ...
                # v=str(view.IMU)
                # ls=v[1:-1]
                # line.append(ls)  # it is  UncompAccel + UncompGyro
                #---UncompAccel
                v=str(view.UncompAccel)
                ls=v[1:-1]
                line.append(ls)
                #---UncompMag
                v=str(view.UncompMag)
                ls=v[1:-1]
                line.append(ls)
                #---UncompGyro
                v=str(view.UncompGyro)
                ls=v[1:-1]
                line.append(ls)
                #---Temp
                line.append(str(view.Temp))
                #---Pres
                line.append(str(view.Pres))
                #---DeltaTheta
                v=str(view.DeltaTheta)
                ls=v[1:-1]
                line.append(ls)
                #---DeltaV
                v=str(view.DeltaV)
                ...
                with open(log_file_name, 'a') as w1:
                    ...
                    current_sample_id+=1
                    if(current_sample_id >= number_of_samples_each_file): #limiting size of file
                        current_sample_id=0
                        w1.close()

            time.sleep(seconds_between_samples) #sampling data once a second
